...
for i in x3 :
    x0 = i
    root = sy.optimize.fsolve(func, 20)
    # root2 = sy.optimize.fsolve(func2, 10)
    if root[0] < 0 :
        root[0] = 0
        if AC_T == 0 :
...
#     if Difference_y < 0.1 and Difference_x > 0.02 :
...
plt.plot(x22, y22, color='red', label='$\Omega=30\epsilon$',linewidth=3)
plt.plot(x33, y33, color='green', label='$\Omega=20\epsilon$',linewidth=3)
plt.plot(x44, y44, color='blue', label='$\Omega=10\epsilon$',linewidth=3)
...
plt.xlabel("$\phi_0$", fontdict={'size': 18}) #title of abscissa
plt.ylabel("$\mu_b/\epsilon$", fontdict={'size': 18}) #title of ordinate
